...
const switchLogsVolumeAction = createAction<{ exploreId: ExploreId; enabled: boolean }>(
  'explore/switchLogsVolumeAction'
);
...
export function switchLogsVolume(exploreId: ExploreId, enabled: boolean): ThunkResult<void> {
  return (dispatch, getState) => {
    dispatch(switchLogsVolumeAction({ exploreId, enabled }));
...
export const queryReducer = (state: ExploreItemState, action: AnyAction): ExploreItemState => {
  ...
  if (switchLogsVolumeAction.match(action)) {
